...
export default function Navbar() {
  ...
  return (
    <>
      ...
      <div className="bg-[#333333] text-slate-200 text-xs py-2 px-4 border-b border-slate-700/60">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-2">
          ...
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 sm:gap-5">
            {contactInfo.phone1 && (
              <a href={`tel:${contactInfo.phone1.replace(/\s+/g, '')}`} className="flex items-center gap-1.5 hover:text-[#72AFDB] transition-colors">
                <Phone className="w-3.5 h-3.5 text-[#72AFDB]" />
                <span>{contactInfo.phone1}</span>
              </a>
            )}
            {contactInfo.phone2 && (
              <a href={`tel:${contactInfo.phone2.replace(/\s+/g, '')}`} className="flex items-center gap-1.5 hover:text-[#72AFDB] transition-colors">
                <Phone className="w-3.5 h-3.5 text-[#72AFDB]" />
                <span>{contactInfo.phone2}</span>
              </a>
            )}
            {contactInfo.email1 && (
              <a href={`mailto:${contactInfo.email1}`} className="hidden sm:flex items-center gap-1.5 hover:text-[#72AFDB] transition-colors">
          ...
          <div className="flex items-center justify-center gap-4 text-slate-300">
            <span className="hidden lg:inline-block text-[11px] font-medium text-slate-300 border-r border-slate-700 pr-4">
            ...
            <div className="flex items-center gap-2.5 text-slate-300">
              {contactInfo.socialLinkedin && (
                <a href={contactInfo.socialLinkedin} target="_blank" rel="noopener noreferrer" className="hover:text-[#72AFDB] transition-colors" title="LinkedIn">
                  <Linkedin className="w-3.5 h-3.5" />
                </a>
              )}
              {contactInfo.socialFacebook && (
                <a href={contactInfo.socialFacebook} target="_blank" rel="noopener noreferrer" className="hover:text-[#72AFDB] transition-colors" title="Facebook">
                  <Facebook className="w-3.5 h-3.5" />
                </a>
              )}
              {contactInfo.socialInstagram && (
                <a href={contactInfo.socialInstagram} target="_blank" rel="noopener noreferrer" className="hover:text-[#72AFDB] transition-colors" title="Instagram">
                  <Instagram className="w-3.5 h-3.5" />
                </a>
              )}
              {contactInfo.socialYoutube && (
                <a href={contactInfo.socialYoutube} target="_blank" rel="noopener noreferrer" className="hover:text-[#72AFDB] transition-colors" title="YouTube">
                  <Youtube className="w-3.5 h-3.5" />
                </a>
              )}
            </div>

            <div className="pl-2 border-l border-slate-700 shrink-0">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            ...
            <Link href="/" className="shrink-0">
              <Logo size="md" theme="auto" showIcon={false} />
            </Link>
