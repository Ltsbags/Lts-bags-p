...
export default function LanguageSelector({ variant = 'header' }: LanguageSelectorProps) {
  ...
  const handleQuickFlagClick = (code: string) => {
    setLanguage(code);
    setIsOpen(false);
  };

  // Reusable Google Translate Style Box Component
  const GoogleTranslateWidgetBox = ({ isCompact = false }: { isCompact?: boolean }) => (
    <div className="flex flex-col items-center justify-center space-y-2 p-3 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-lg text-center max-w-xs mx-auto">
        ...
        {quickFlags.map((item) => {
          ...
          return (
            <button
              key={item.code}
              type="button"
              onClick={() => handleQuickFlagClick(item.code)}
              title={item.label}
              className={`text-lg hover:scale-125 transition-transform p-0.5 rounded cursor-pointer ${
                isSelected ? 'ring-2 ring-sky-500 scale-110 font-bold' : 'opacity-80 hover:opacity-100'
              }`}
            >
              {item.flag}
            </button>
      ...
      <div className="relative w-full max-w-[200px]">
        <select
          ...
          className="w-full appearance-none bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-slate-100 text-xs font-semibold py-1.5 pl-3 pr-8 rounded-md shadow-2xs hover:border-sky-500 focus:outline-none focus:ring-2 focus:ring-sky-500 cursor-pointer text-center"
        >
  ...
  if (variant === 'widget') {
    return <GoogleTranslateWidgetBox />;
  }
  ...
  return (
    <div className="relative inline-block text-left" ref={dropdownRef}>
      ...
      >
        <LanguagesIcon className="w-3.5 h-3.5 text-sky-400 shrink-0" />
